...
export async function tick(t: Date = now()): Promise<TickResult> {
  ...
  // §16.1/§16.2 — anchored to delivery date, per order.
  ...
  // so any newer order resets the timer automatically.
  const customers = await db.customer.findMany({ where: { marketingOptOut: false, lastOrderAt: { not: null } } });
